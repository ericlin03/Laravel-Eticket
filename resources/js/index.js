...
import DepositeArtifact from "../../build/contracts/Deposite.json";
...
const App = {
            ...
            if (checkStatus) {
                ...
                setTimeout(function() {
                    window.location.href =
                        "http://localhost:8000/confirmTicket";
                }, 200);
            }
        } catch (error) {
            console.log(error);
    ...
    checkStatus: async function() {
        if (this.account == $("#wallet").text()) {
            ...
            alert(
                "錢包地址與個人資料不相符，請確認MetaMask帳號與付款地址相同，確認完後請重新整理。"
            );
        }
    },
    ...
    allBlock: async function() {
        try {
            ...
            for (var i = blockNumber; i >= 0; i--) {
                web3.eth.getBlock(i, function(err, object) {
                    if (!err) {
                        $("#allBlockBody").append(
                            "<tr><td># " +
                                object.number +
    ...
    specificBlock: async function() {
        try {
            ...
            web3.eth.getBlock(blockNum, function(err, object) {
                if (!err) {
                    $("#specificHead").replaceWith(
                        '<th id="specificHead">' + blockNum + "</th>"
                    );
                    $("#hash").replaceWith(
                        '<td id="hash">' + object.hash + "</td>"
                    );
                    $("#extraData").replaceWith(
                        '<td id="extraData">' + object.extraData + "</td>"
                    );
                    $("#transactions").replaceWith(
                        '<td id="transactions">' + object.transactions + "</td>"
                    );
                    console.log(object);
                }
            });
    ...
    searchTx: async function() {
        try {
            ...
            web3.eth.getTransactionReceipt(txHash, function(err, object) {
                if (!err) {
                    $("#txHead").replaceWith(
                        '<th id="txHead">' + txHash + "</th>"
                    );
                    $("#blockNumber").replaceWith(
                        '<td id="blockNumber">' + object.blockNumber + "</td>"
                    );
                    $("#from").replaceWith(
                        '<td id="from">' + object.from + "</td>"
                    );
                    $("#to").replaceWith('<td id="to">' + object.to + "</td>");
                    $("#contractAddress").replaceWith(
                        '<td id="contractAddress">' +
                            object.contractAddress +
                            "</td>"
                    );
                    $("#gasUsed").replaceWith(
                        '<td id="gasUsed">' + object.gasUsed + " wei</td>"
                    );
                    console.log(object);
                }
            });
